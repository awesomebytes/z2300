...
for f in os.listdir('.'):
    if f.upper().endswith('JPG'):
        ...
        if im_size != (3648, 2736) and im_size != (2736, 3648):
            ...
            into.paste(
                im, (int((into.size[0] - im.size[0]) / 2), int((into.size[1] - im.size[1]) / 2)))
            into.save(f)
            im = into
        ...
        source_image = pyexiv2.Image(f)
        exif_data = source_image.read_exif()

        try:
            exif_width = int(
                exif_data["Exif.Photo.PixelXDimension"])
            exif_height = int(
                exif_data["Exif.Photo.PixelYDimension"])
        except KeyError:
            exif_width = None
            exif_height = None
...
#      print k,v
        ...
        if exif_width != im_size[0] or exif_height != im_size[1]:
            ...
            # Must have this size anyways
            source_image.modify_exif({'Exif.Photo.PixelXDimension': '3648',
                                        'Exif.Photo.PixelYDimension': '2736'})
            source_image.close()
